...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const isDeliveryMode = activeTab === 'delivery';
  ...
  return (
    <div className="min-h-screen bg-canvas flex flex-col font-sans antialiased text-panadero">
      ...
      <main
        className={`flex-1 ${
          isKitchenMode
            ? 'p-2 sm:p-4 md:p-6 bg-slate-900 text-slate-100'
            : isDeliveryMode
            ? 'w-full max-w-3xl mx-auto px-2.5 sm:px-4 py-3 sm:py-6 pb-24'
